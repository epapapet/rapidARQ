...
static class CARQTxClass: public TclClass {
 public:
	CARQTxClass() : TclClass("CARQTx") {}
	TclObject* create(int, const char*const*) {
		return (new CARQTx);
	}
} class_CARQ_tx;

static class CARQAckerClass: public TclClass {
 public:
	CARQAckerClass() : TclClass("CARQAcker") {}
	TclObject* create(int, const char*const*) {
		return (new CARQAcker);
	}
} class_CARQ_acker;

static class CARQNackerClass: public TclClass {
 public:
	CARQNackerClass() : TclClass("CARQNacker") {}
	TclObject* create(int, const char*const*) {
		return (new CARQNacker);
	}
} class_CARQ_nacker;
...
CARQTx::CARQTx() : arqh_(*this)
{
	...
	num_pending_retrans_ = 0; //number of retransmissions scheduled and pending in CARQTx
  ...
  ranvar_ = NULL; // random number generator used for simulating the loss rate for ACKs
	...
	start_time = -1; //time when 1st packet arrived at CARQTx::recv
    ...
    if (strcmp(argv[1], "ranvar") == 0) {
      ...
      if (wnd_ == 0){
        ...
        wnd_ = crounds * atoi(argv[3]);
			...
			sn_cnt = 4 * wnd_; //although 2*wnd_ is enough, we use 4*wnd_ or more to tackle the case that CARQTx drops packets and advances its window without CARQRx knowing
			pkt_buf = new Packet* [wnd_]; //buffer for storing pending frames
			status = new CARQStatus[wnd_]; //the status for each frame: IDLE,SENT,ACKED,RTX,RTXPRE,DROP
      ...
      timeout_events = new CARQTimeoutEvent* [sn_cnt]; //buffer for storing pointers to timeout events
...
void CARQTx::recv(Packet* p, Handler* h)
{

	//This procedure is invoked by the queue_ (i.e., output queue) to deliver a message to CARQTx
  ...
  if (handler_ != 0) printf("\b\b\b\b\b\b\b\b\b\b%10.4f", Scheduler::instance().clock());
	...
	//Sanity checks---------//
	if (&arqh_==0) {
		fprintf(stderr, "Error at CARQTx::recv, Cannot transmit when &arqh_ is Null.\n");
		abort();
	}
	if ((status[most_recent_sq_%wnd_] != IDLE) && ((HDR_CMN(p))-> opt_num_forwards_ >= 0)) {
		fprintf(stderr, "Error at CARQTx::recv, storing position should be in IDLE mode.\n");
		abort();
	}
	//---------------------//
  ...
  if ((blocked_) && ((HDR_CMN(p))-> opt_num_forwards_ >= 0)) {
    if (pending) { fprintf(stderr, "Error at CARQTx::recv, Tx should not have a pending frame when recv is called.\n"); abort(); }
    pending = p; return;
  }
  if ((blocked_) && ((HDR_CMN(p))-> opt_num_forwards_ < 0)) {
    if (coded) { fprintf(stderr, "Error at CARQTx::recv, Tx should not have a pending frame when recv is called.\n"); abort(); }
    coded = p; return;
  }
	...
	if (ch-> opt_num_forwards_ >= 0){
		...
		if (pkt_buf[most_recent_sq_%wnd_]) {fprintf(stderr, "Error at CARQTx::recv, storing position found non empty.\n"); abort();} //pkt_buf is cleared by reset_lastacked
    ...
    CARQTimeoutEvent *etout = new CARQTimeoutEvent();
    etout->type = TIMEOUT;
    etout->sn = most_recent_sq_;
    etout->uid = ch->uid();
    etout->expirationTime = Scheduler::instance().clock() + timeout_;
    etout->isCancelled = false;
    if (timeout_events[most_recent_sq_]) { fprintf(stderr, "Error at CARQTx::recv, found non-empty position of timeout event.\n"); abort(); }
	...
	if (debug) printf("CARQTx, recv: Sending pkt %d. SimTime=%.8f.\n", ch-> opt_num_forwards_, Scheduler::instance().clock());

	blocked_ = 1;
	if (ch-> opt_num_forwards_ >= 0){
		//In general, when sending a native frame we create a copy (even in retransmissions) to decouple CARQTx and CARQRx
		//CARQTx is responsible for freeing frames sent from queue while CARQRx delivers frames to upper layers and frees frames not eventually delivered
		packets_sent += 1;
		Packet *pnew = p->copy();
		send(pnew,&arqh_);
	} else {
...
Packet* CARQTx::create_coded_packet(){ //create a new coded packet with seq nums of <=k previous packets sent
...
/*void CARQTx::ack(int rcv_sn, int rcv_uid)
{
	...
	//Sanity checks--------//
	if (status[rcv_sn%wnd_] == IDLE) { fprintf(stderr,"Error at CARQTx::ack, an ACK is received when the status is not SENT. It is %d.\n", status[rcv_sn%wnd_]); abort(); }
	if (handler_ == 0) { fprintf(stderr,"Error at CARQTx::ack, handler_ is null.\n"); abort(); }
	//--------------------//

	if (status[rcv_sn%wnd_] != ACKED){
		if (debug) printf("CARQTx ack: Pkt %d with status %d is ACKED. The news status is %d. Pending retrans=%d. LA(MR) before is %d(%d). SimTime=%.8f.\n ", rcv_sn, status[rcv_sn%wnd_], ACKED, num_pending_retrans_, last_acked_sq_, most_recent_sq_, Scheduler::instance().clock());
...
void CARQTx::parse_cumulative_ack(Packet *p){ //called when a coded ack arrives, reads the contents and checks to advance the window
  ...
  bool valid_seqnum = true;
	...
	if (debug) {
		int readint = -1;
		printf("CARQTx, parse_cumulative_ack: Received ACK contains =");
...
bool CARQTx::parse_ack(int rcv_sn, bool batch)
{
  ...
  int fw_dis = (rcv_sn - last_acked_sq_ + sn_cnt)%sn_cnt;
	...
	if (status[rcv_sn%wnd_] != ACKED){
		...
		if ((status[rcv_sn%wnd_] == RTX) || (status[rcv_sn%wnd_] == RTXPRE)) num_pending_retrans_--; //reduce the number of scheduled retransmissions
		status[rcv_sn%wnd_] = ACKED;
    if (timeout_events[rcv_sn]) (timeout_events[rcv_sn])->expirationTime = -1.0; //the timeout event may be NULL if pkt is DROPPED and a late decode ACK is received
...
bool CARQTx::parse_nack(int rcv_sn)
{
  ...
  //Sanity checks--------//
	if (status[rcv_sn%wnd_] == IDLE) { fprintf(stderr,"Error at CARQTx::parse_nack when the status is not SENT.\n"); abort(); }
	if (handler_ == 0) { fprintf(stderr,"Error at CARQTx::parse_nack, handler_ is null\n"); abort(); }
	if (&arqh_==0) { fprintf(stderr, "Error at CARQTx::parse_nack, Cannot transmit when &arqh_ is Null.\n"); abort(); }
	//--------------------//
	...
	if (status[rcv_sn%wnd_] != SENT) { //it is possible to receive an NACK for a frame acked through a cumulative ACK (i.e., coded frame) when the cumulative ACK arives after the retransmission
		if (debug) printf("CARQTx, parse_nack for pkt %d with status %d. Ignoring. SimTime=%.8f.\n", rcv_sn, status[rcv_sn%wnd_], Scheduler::instance().clock());
		return false;
	} else {
		if (pkt_buf[rcv_sn%wnd_] == NULL) { fprintf(stderr,"Error at CARQTx::parse_nack but pkt is not found.\n"); abort(); }
		if (debug) printf("CARQTx, parse_nack: timeout_expired for pkt %d with status %d. SimTime=%.8f.\n", rcv_sn, status[rcv_sn%wnd_], Scheduler::instance().clock());
	}

	if ( num_rtxs[rcv_sn%wnd_] < retry_limit_) { //packet should be retransmitted
		status[rcv_sn%wnd_] = RTXPRE;
		if (!blocked_){ //if CARQTx is available go on with retransmission
			if (debug) printf("CARQTx, parse_nack: Sending pkt %d. SimTime=%.8f.\n", rcv_sn, Scheduler::instance().clock());
			blocked_ = 1;
    ...
    return false;
		...
		if (debug) printf("CARQTx, nack: Dropping pkt %d. SimTime=%.8f.\n", rcv_sn, Scheduler::instance().clock());
...
void CARQTx::timeout_expired(int rcv_sn, int rcv_uid)
{
	...
	//Sanity checks--------//
	if (status[rcv_sn%wnd_] == IDLE) { fprintf(stderr,"Error at CARQTx::timeout_expired when the status is not SENT.\n"); abort(); }
	if (handler_ == 0) { fprintf(stderr,"Error at CARQTx::timeout_expired, handler_ is null\n"); abort(); }
	if (&arqh_ == 0) { fprintf(stderr, "Error at CARQTx::timeout_expired, Cannot transmit when &arqh_ is Null.\n"); abort(); }
	//--------------------//

	if (status[rcv_sn%wnd_] != SENT) { //it is possible to receive an NACK for a frame acked through a cumulative ACK (i.e., coded frame) when the cumulative ACK arives after the retransmission
		if (debug) printf("CARQTx, timeout_expired for pkt %d with status %d. Ignoring. SimTime=%.8f.\n", rcv_sn, status[rcv_sn%wnd_], Scheduler::instance().clock());
		return;
	} else {
		if (pkt_buf[rcv_sn%wnd_] == NULL) { fprintf(stderr,"Error at CARQTx::timeout_expired but pkt is not found.\n"); abort(); }
		if (debug) printf("CARQTx, timeout_expired: timeout_expired for pkt %d with status %d. SimTime=%.8f.\n", rcv_sn, status[rcv_sn%wnd_], Scheduler::instance().clock());
	}

	if ( num_rtxs[rcv_sn%wnd_] < retry_limit_) { //packet should be retransmitted
		status[rcv_sn%wnd_] = RTX;
		if (!blocked_){ //if CARQTx is available go on with retransmission
			if (debug) printf("CARQTx, timeout_expired: Sending pkt %d. SimTime=%.8f.\n", rcv_sn, Scheduler::instance().clock());
			blocked_ = 1;
			num_rtxs[rcv_sn%wnd_]++;
			status[rcv_sn%wnd_] = SENT;
      vector<int>::iterator it = std::find(most_recent_sent.begin(), most_recent_sent.end(), rcv_sn);
...
void CARQTx::resume()
{
	//This procedure is invoked by link_ when a transmission is completed, i.e., it is invoked T secs after CARQTx executes send() where T equals transmission_delay.
	blocked_ = 0;
  ...
  if (coded){
    ...
    native_counter++;
		...
		send(pnew,&arqh_);
	} else {//there are no pending retransmissions, check whether it is possible to send a new packet
		//TO DO: check whether active window reaches wnd_ and CARQTx is stuck without asking queue for the next frame
...
int CARQTx::findpos_retrans()
{
	//----------DEBUG------------------//
	if((last_acked_sq_+1)%sn_cnt == most_recent_sq_) { fprintf(stderr, "Error at CARQTx::findpos_retrans, no packet is waiting (stored) for transmission.\n"); abort(); }
	//---------------------------------//
  ...
  int runner_wide_ = (last_acked_sq_+1)%sn_cnt;
	...
	//----------DEBUG------------------//
	if (!found){ fprintf(stderr, "Error at CARQTx::findpos_retrans, packet with RTX or RTXPRE status NOT FOUND.\n"); abort(); }
	//---------------------------------//

	return runner_wide_;
} //end of findpos_retrans

void CARQTx::reset_lastacked()
{
  ...
  int wide_runner_ = (last_acked_sq_+1)%sn_cnt;
    ...
    if(it != most_recent_sent.end()){ //if seq_num found in most_recent_sent, remove elements with the same value
      most_recent_sent.erase(it);
    }
  ...
  if (window_advanced) {
	...
	if(debug) printf("CARQTx, reset_lastacked: new LA(MR) are %d(%d). SimTime=%.8f.\n", last_acked_sq_, most_recent_sq_, Scheduler::instance().clock());

} // end of reset_lastacked

void CARQTx::handle(Event* e){

  CARQEvent *received_event = (CARQEvent *)e;
  CARQEventTypes rcv_type = received_event->type;
  if (rcv_type == TIMEOUT){ //handle a timeout expiration event

    CARQTimeoutEvent *received_tevent = (CARQTimeoutEvent *)e;
    ...
    //because we need to be sure that the event is not in the Scheduler (it is added when the packets are sent for the first time).
...
int CARQRx::command(int argc, const char*const* argv)
{
	Tcl& tcl = Tcl::instance();
  if (argc == 2){
...
int CARQAcker::command(int argc, const char*const* argv)
{
	Tcl& tcl = Tcl::instance();
	if (argc == 3) {
		if (strcmp(argv[1], "setup-wnd") == 0) {
      ...
      if (wnd_ == 0){
        ...
        wnd_ = crounds * arq_tx_->get_ratek();
			...
			sn_cnt = 4 * wnd_; //never less than 2*wnd_ but usually more than that (see note in CARQTx command)
			...
			status = new CARQPacketStatus[wnd_]; //the status of each pending frame: NONE,MISSING,RECEIVED,DECODED
			for(int i=0; i<wnd_; i++){ pkt_buf[i] = NULL; status[i] = NONE;}
			for(int i=0; i<sn_cnt; i++){ lost_pkt_buf[i] = NULL; }
			return(TCL_OK);
		}
		if (strcmp(argv[1], "setup-CARQNacker") == 0) {
			if (*argv[2] == '0') {
				tcl.resultf("Cannot attach NULL CARQNacker\n");
				return(TCL_ERROR);
			}
			nacker = (CARQNacker*)TclObject::lookup(argv[2]);
			return(TCL_OK);
		}
    if (strcmp(argv[1], "setup-filename") == 0) {
      switch (atoi(argv[2])){
        case 1:
          filename = "arq/results/carq_lr_rtt.txt";
          break;
        case 2:
          filename = "arq/results/carq_cr_lr.txt";
          break;
        case 3:
          filename = "arq/results/carq_rtt.txt";
          break;
        case 4:
	...
	} return CARQRx::command(argc, argv);
} //end of command

void CARQAcker::recv(Packet* p, Handler* h)
{
	...
	bool within_bww = (bw_dis < wnd_) ? (true) : (false); //bool to indicate whether the new frame is (or not) within the CARQTx's active window as seen by CARQRx (backward window)
	int oldest_sq_sender = (most_recent_acked - wnd_ + 1 + sn_cnt)%sn_cnt; //the lower bound of CARQTx's active window as seen by CARQRx before receiving seq_num
	int new_oldest_sq_sender = (seq_num - wnd_ + 1 + sn_cnt)%sn_cnt; //the lower bound of CARQTx's active window as seen by CARQRx after receiving seq_num
  ...
  bool decoding_occured = false; //bool to check if the new arrival created a decoding
	...
	if (within_fww){//frame belongs to the forward window
		...
		if(seq_num == nxt_seq){ //frame arrives in order, ack and send frame and finally check if other frames are now in order
			if (debug) printf("CARQRx, recv1 (LF=%d,MR=%d): received pkt %d. SimTime=%.8f.\n", last_fwd_sn_, most_recent_acked, seq_num, Scheduler::instance().clock());
			status[nxt_seq%wnd_] = NONE;
			if (pkt_buf[nxt_seq%wnd_]){ fprintf(stderr, "Error at CARQRx::recv, position of in order frame found non empty.\n"); abort(); }
      ...
      if (last_delay_sample < min_delay) min_delay = last_delay_sample;
			...
			if ((status[seq_num%wnd_] != RECEIVED) && (status[seq_num%wnd_] != DECODED)) {
				if (debug) printf("CARQRx, recv2 (LF=%d,MR=%d): received pkt %d. SimTime=%.8f.\n", last_fwd_sn_, most_recent_acked, seq_num, Scheduler::instance().clock());
				status[seq_num%wnd_] = RECEIVED;
				if (pkt_buf[seq_num%wnd_]){ fprintf(stderr, "Error at CARQRx::recv, position of not received and not decoded frame within forward window found non empty.\n"); abort(); }
        ...
        if (lost_pkt_buf[seq_num]) Packet::free(lost_pkt_buf[seq_num]);
				...
				//Sanity-------------------------//
				if ((HDR_CMN(pkt_buf[seq_num%wnd_]))->uid() != (HDR_CMN(p))->uid()){ fprintf(stderr, "Error at CARQRx::handle, received retransmission has not the same uid.\n"); abort(); }
				Packet::free(p);
				//-------------------------------//
			}
		}
		if (fw_dis > fw_width) { //the new frames is beyond the most_recent_acked
			...
			clean_decoding_matrix(oldest_sq_sender, new_oldest_sq_sender); //remove from the coding structures the frames that are now out of CARQTx's active window
      //clean_known_packets(oldest_sq_sender, new_oldest_sq_sender); //remove frames that are out of ARQTx's active wnd
		}

	} else if (within_bww && !within_fww) {//frame belongs to the backward window (i.e., CARQTx's active window), so it is a retransmitted frame due to loss of ACK
		...
		//This happens when one or more frames are dropped by CARQTx (when exceeding the retrans limit) so it moves on to transmit new frames.
		//CARQRx is not informed to advance its window and receives brand new frames that may consider as old ones
		//Using 2*wnd_ sequence numbers (= forward window + backward window) is enough for identifying new frames from retransmissions provided that CARQTx advances its window only after an ACK by CARQRx
		//When CARQTx can advance its window and send new frames after dropping a old frame then we need more than 2*wnd_ sequence numbers to identify those the new frames because otherwise those new frames will appear to be in the backward window and will be mistaken by CARQRx as retransmission frames
		if (debug) printf("CARQRx, recv3 (LF=%d,MR=%d): received pkt %d. SimTime=%.8f.\n", last_fwd_sn_, most_recent_acked, seq_num, Scheduler::instance().clock());
		...
		status[seq_num%wnd_] = RECEIVED;
		if (pkt_buf[seq_num%wnd_]){ fprintf(stderr, "Error at CARQRx::recv, position of new frame beyond most recent found non empty.\n"); abort(); }
		pkt_buf[seq_num%wnd_] = p;
    if (lost_pkt_buf[seq_num]) Packet::free(lost_pkt_buf[seq_num]);
		lost_pkt_buf[seq_num] = NULL;
		...
		most_recent_acked = seq_num;
		clean_decoding_matrix(oldest_sq_sender, new_oldest_sq_sender); //remove from the coding structures the frames that are now out of CARQTx's active window
    //clean_known_packets(oldest_sq_sender, new_oldest_sq_sender);
	}
...
void CARQAcker::deliver_frames(int steps, bool mindgaps, Handler *h)
{
...
void CARQAcker::print_parameters(double err, double ack, double sim_time, int seed)
{
  printf("Protocol:"); printf("\033[0;32m"); printf("\t\t\tC-ARQ\n"); printf("\033[0m");
  printf("------------------------------------------------------\n");
  ...
  printf("Coding depth (window):");printf("\t\t%d(%d)\n",  arq_tx_->get_coding_depth(),  arq_tx_->get_ratek()*arq_tx_->get_coding_depth());printf("\033[0m");
...
void CARQAcker::print_stats(double err, double ack, double sim_time, int seed)
{
  printf("\n------------------ STATS FOR C-ARQ -------------------\n");
  ...
  if(access("arq/results", F_OK) != 0)
	{
    char command[256];
    sprintf(command, "mkdir -p %s", "arq/results");
    system(command);
  }
  FILE *fp;
  fp = fopen(filename, "a+");
  ...
  if(ftell(fp) == 0) {
    ...
    //sprintf(command_two, "wc -l < %s", "arq/results/carq.txt");
    fgets(numLines, 256, popen(command_two, "r"));
    cntLines = (int)strtoull(numLines, NULL, 10);
  }
  fprintf(fp, "%d %.0f %.3f  %d  %.0f  %d  %.3f  %.3f  %d  %d  %d  %.3f  %.0f  %d  %f %f  %.0f  %.3f  %f  %f  %.0f  %.0f  %f  %f  %f  %f  %f  %f  %.0f  %f  %f  %0.f  %f  %0.f  %f  %0.f\n", cntLines, arq_tx_->get_linkbw(), arq_tx_->get_linkdelay(), wnd_, arq_tx_->get_linkbw(), arq_tx_->get_apppktsize(), err, ack, arq_tx_->get_retry_limit(), arq_tx_->get_ratek(), arq_tx_->get_coding_depth(), arq_tx_->get_timeout(), sim_time, seed, arq_tx_->get_start_time(), finish_time, delivered_pkts, delivered_data/1048576, throughput * 1.0e-6, arq_tx_->get_total_pause_time(), arq_tx_->get_total_packets_sent(), arq_tx_->get_total_coded_packets_sent(), mean * 1.0e+3, max_delay * 1.0e+3, min_delay * 1.0e+3, meanjitter * 1.0e+3, avg_rtxs, packet_loss_rate, num_of_decodings, avg_num_of_decoded_pkts, average_dec_matrix_size, max_dec_matrix_size, average_inv_known_pkts_size, max_inv_known_pkts_size, average_known_pkts_size, max_known_pkts_size);
  //fprintf(fp, "%.0f %.3f  %d  %.0f  %d  %.3f  %.3f  %d  %d  %d  %.3f  %.0f  %d  %f %f  %.0f  %.3f  %f  %f  %.0f  %.0f  %f  %f  %f  %f  %f  %f  %.0f  %f  %f  %0.f  %f  %0.f  %f  %0.f\n", arq_tx_->get_linkbw(), arq_tx_->get_linkdelay(), wnd_, arq_tx_->get_linkbw(), arq_tx_->get_apppktsize(), err, ack, arq_tx_->get_retry_limit(), arq_tx_->get_ratek(), arq_tx_->get_coding_depth(), arq_tx_->get_timeout(), sim_time, seed, arq_tx_->get_start_time(), finish_time, delivered_pkts, delivered_data/1048576, throughput * 1.0e-6, arq_tx_->get_total_pause_time(), arq_tx_->get_total_packets_sent(), arq_tx_->get_total_coded_packets_sent(), mean * 1.0e+3, max_delay * 1.0e+3, min_delay * 1.0e+3, meanjitter * 1.0e+3, avg_rtxs, packet_loss_rate, num_of_decodings, avg_num_of_decoded_pkts, average_dec_matrix_size, max_dec_matrix_size, average_inv_known_pkts_size, max_inv_known_pkts_size, average_known_pkts_size, max_known_pkts_size);
  fclose(fp);
} //end of print_stats

void CARQAcker:: parse_coded_packet(Packet *cp, Handler* h){ //function that reads a coded packet and update the list with known packets
  ...
  bool found_first_lost = false;
	...
	if (debug) {
		...
		if (start_pos != stop_pos){
			printf("CARQRx, parse_coded_packet: The window contains: ");
			do{
			...
			printf("CARQRx, parse_coded_packet: The window contains nothing.\n");
		}
		set<int>:: iterator it;
		printf("CARQRx, parse_coded_packet: The known_pkts are: ");
		for(it = involved_known_packets.begin(); it != involved_known_packets.end(); ++it){
			printf("%d, ", *(it));
		}
		printf(".\n");
    printf("CARQRx, parse_coded_packet: The involved_known_pkts are: ");
		for(it = involved_known_packets.begin(); it != involved_known_packets.end(); ++it){
			printf("%d, ", *(it));
		}
		printf(".\n");
		printf("CARQRx, parse_coded_packet: The lost_pkts are: ");
	...
	if (debug) printf("CARQRx, parse_coded_packet: The coded pkt contains: ");
  ...
  if ((intersect.size() == 0) || (coded_packets.size() == 0)){
    //If the received coded pkt resets lost_packets and coded_packets polls or if a decoding occured we need to reset involved_known_packets
...
bool CARQAcker::decode(Handler* h, bool afterCodedreception){

  if (debug) printf("CARQRx, decode : Examining decoding (LPKTS=%d,RCODED=%d).\n", (int)lost_packets.size(), (int)coded_packets.size());
  ...
  if (decoding_is_possible){
      ...
      known_packets.insert(*it); //move decoded frame into known ones
  		if (debug) printf("CARQRx, decode: decoding pkt %d.\n", lost_sn);
  		//------------------------------------------//
  		//if ((status[lost_sn%wnd_] != MISSING)) { fprintf(stderr, "Error at CARQRx::decode, decoded pkt cannot be found or has a wrong status.\n"); abort();}
  		//------------------------------------------//
          ...
          sum_of_delay = sum_of_delay + Scheduler::instance().clock() - HDR_CMN(pkt_buf[lost_sn%wnd_])->ts_arr_;
  			...
  			if (fw_dis > fw_width) { //if the decoded frame is beyond the most_recent_acked
  				...
  				clean_decoding_matrix(oldest_sq_sender, new_oldest_sq_sender); //remove from the coding structures the frames that are now outside the backward window (i.e., CARQTx's active window)
          //clean_known_packets(oldest_sq_sender, new_oldest_sq_sender);
        }
...
Packet* CARQAcker::create_coded_ack(){
  ...
  for (itr = lost_packets.begin(); itr != lost_packets.end(); itr++)
    ...
    if (seen_packets.count(*itr) != 0) sent_sq_num = *itr; //seen packets are positively acknowledged
...
void CARQAcker::clean_decoding_matrix(int from, int to)
{
  int runner_ = from;
	do {
...
/*void CARQAcker::clean_known_packets(int from, int to){
  int runner_ = from;
	do {
...
void CARQAcker::delete_lost_and_associated_coded_from_matrix(int pkt_to_remove)
{
  ...
  //In doing so we also need to delete all coded pkts containing the deleted lost pkt because they will no be useful for decodings
  ...
  if (deleted_lost_pkt > 0){
    ...
    for (itcodedpkts = coded_packets.begin(); itcodedpkts != coded_packets.end(); ++itcodedpkts){
        ...
        if (erase_coded_cnt > 0){ //coded pkt contains the deleted lost one
          ...
          if (intersect.size() == 0) {fprintf(stderr, "Error at CARQRx::delete_lost_and_associated_coded_from_matrix, found coded pkt that does contain any lost pkt.\n"); abort();}
          intersect.clear();
...
void CARQAcker::delete_lost_and_find_associated_coded_in_matrix(int pkt_to_remove)
{
  //Should delete a lost_packet that turned into known (through a retransmission)
  //In doing so we also need to delete all coded pkts containing only this lost_packet because they are no more useful in decodings
  //We do not need to update involved_known_packets: we could delete involved_known_packets that are only contained in the deleted coded ones but
  //the impact in reducing the size of involved_known_packets will be minimal and processing overhead high
  vector<vector<int> >::iterator itcodedpkts;
  vector<vector<int> > temp_coded;
...
void CARQAcker::delete_known_from_matrix(int pkt_to_remove){
...
void CARQAcker::delete_known_from_matrix_strict(int pkt_to_remove){
...
} //end of delete_known_from_matrix_srict

void CARQAcker::repopulate_seen_packets(){
...
void CARQAcker::log_lost_pkt(Packet *p){
  ...
  if (ch->opt_num_forwards_ < 0){ seq_num = seq_num + 10000; rpktuid = ch->aomdv_salvage_count_; }
	...
	if (debug) printf("CARQRx, log_lost_pkt: Pkt %d is lost.\n", seq_num);
	if (debug) printf("CARQRx, log_lost_pkt: Logging pkt %d. Position %d.\n", seq_num, seq_num%wnd_);

	if (lost_pkt_buf[seq_num]){
		hdr_cmn *chold = HDR_CMN(lost_pkt_buf[seq_num]);
		if (rpktuid != chold->uid()){ //this is an old frame, so delete it
      if (ch->opt_num_forwards_ < 0) { fprintf(stderr, "Error at CARQAcker::log_lost_pkt, coded received instead of native but native is missing.\n"); abort(); } //a coded pkt cannot find a pkt that is not the corresponding native
...
CARQNacker::CARQNacker()
{
	debug = false;
	acker = NULL;
} //end of CARQNacker

int CARQNacker::command(int argc, const char*const* argv)
{
	Tcl& tcl = Tcl::instance();
	if (argc == 3) {
		if (strcmp(argv[1], "setup-CARQAcker") == 0) {
			if (*argv[2] == '0') {
				tcl.resultf("Cannot attach NULL CARQAcker\n");
				return(TCL_ERROR);
			}
			acker = (CARQAcker*)TclObject::lookup(argv[2]);
			return(TCL_OK);
		}
	} return CARQRx::command(argc, argv);
} //end of command
...
void CARQNacker::recv(Packet* p, Handler* h)
{
