...
ARQAcker::ARQAcker()
{
    ...
    delivered_pkts = 0; //the number of frames delivered to the upper layer, used for statistic purposes
} //end of constructor
...
void ARQAcker::recv(Packet* p, Handler* h)
{
    ...
    bool sdould_check_for_decoding = true; //bool to indicate whether the new reception should trigger a decoding
    ...
    if (within_fww){//frame belongs to the forward window
       ...
       if (fw_dis > fw_width) { //the new frames is beyond the most_recent_acked
        ...
        sdould_check_for_decoding = false;
    ...
    if (sdould_check_for_decoding){ //check if decoding is new possible
      known_packets.insert(seq_num); //Update data for decoding
      lost_packets.erase(seq_num); //Update data for decoding
      decode(h); //Check if decoding is now possible
    }
...
{
  printf("The total number of delivered pkts is %d.\n", delivered_pkts);
} //end of print_stats
...
void ARQAcker::decode(Handler* h){
    ...
    if ((int)lost_packets.size() <= received_coded_cnt){
      if (lost_packets.size() > 0) decoding_is_possible = true;
...
Packet* ARQAcker::create_coded_ack(){
  ...
  unsigned char *buffer = new unsigned char[sizeof(int)*((int)(lost_packets.size()) + 1)]; //4 bytes for each decoded frame plus a byte for the counter
...
void ARQAcker::log_lost_pkt(Packet *p){
...
void ARQAcker::handle(Event* e)
{
    ...
    if ( ranvar_->value() < err_rate ){
      if (debug) printf("Acker, handler: Packet id is %d.\n", rcv_sn);
  		if (rcv_p == NULL) arq_tx_->nack(rcv_sn, rcv_uid); //a coded ack is not expected so no need to call arq_tx->nack()
  	} else {
