...
void CaterpillarTx::handle(Event* e){
  ...
  if (rcv_type == TIMEOUT){ //handle a timeout expiration event
    ...
    if (received_tevent->isCancelled){ //event is really cancelled (expirationTime <0) or should be rescheduled (expirationTime > current time)

      if (received_tevent->expirationTime < 0){
        //events are deleted by reset_last_acked or by timeout_expired when rtx_limit is reached
...
Packet* CaterpillarAcker::create_coded_ack(){
	...
	unsigned char *buffer = new unsigned char[sizeof(int)*((int)(known_packets.size()) + (int)(seen_packets.size()) + 1)]; //4 bytes for each decoded frame plus a byte for the counter
  ...
  for (itr = seen_packets.begin(); itr != seen_packets.end(); itr++)
	{
    *(buffer+sizeof(int)*(cnt_pkts+1)) =  (*itr >> 24) & 0xFF;
		*(buffer+sizeof(int)*(cnt_pkts+1)+1) = (*itr >> 16) & 0xFF;
		*(buffer+sizeof(int)*(cnt_pkts+1)+2) = (*itr >> 8) & 0xFF;
