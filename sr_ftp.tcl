...
if {$arg_cnt != 11} {
    puts "# usage: ns <scriptfile> <bandwidth> <propagation_delay> <window_size> <pkt_size> <err_rate> <ack_rate> <num_rtx> <timeout> <simulation_time> <seed> <experiment_id>"
...
set receiver [$ns link-arq $window $apppktSize $timeout_period $num_rtx $n1 $n3 [lindex $argv 9] [lindex $argv 5] [lindex $argv 10]]
...
$ns at 0.0 "$receiver print-parameters [lindex $argv 4] [lindex $argv 5] [lindex $argv 8] [lindex $argv 9]"
$ns at 0.0 "$ftp start"
