...
static class TetrysACKReceiverClass: public TclClass {
 public:
	TetrysACKReceiverClass() : TclClass("TetrysACKRx") {}
	TclObject* create(int, const char*const*) {
		return (new TetrysACKRx);
	}
} class_Tetrys_ackrx;
...
void TetrysTx::handle_ack(Packet* p){
...
int TetrysAcker::command(int argc, const char*const* argv)
{
    ...
    if (strcmp(argv[1], "setup-filename") == 0) {
      switch (atoi(argv[2])){
        case 1:
          filename = "arq/results/tetrys_lr_rtt.txt";
          break;
        case 2:
          filename = "arq/results/tetrys_cr_lr.txt";
          break;
        case 3:
          filename = "arq/results/tetrys_rtt.txt";
          break;
        case 4:
...
void TetrysAcker::print_parameters(double err, double ack, double sim_time, int seed)
{

  printf("Protocol:"); printf("\033[0;32m"); printf("\t\t\tTetrys\n"); printf("\033[0m");
  printf("------------------------------------------------------\n");
...
void TetrysAcker::delete_lost_and_affected_coded(set<int> packets_to_remove)
{
  ...
  for (itcodedpkts = coded_packets.begin(); itcodedpkts != coded_packets.end(); ++itcodedpkts){
    ...
    if (intersect.size() != 0){ //if at least one deleted lost pkt is contained in the coded one
      ...
      //if (intersect_sanity.size() == 0) {fprintf(stderr, "Error at TetrysAcker::delete_lost_and_affected_coded, found coded pkt that does not contain any lost pkt.\n"); abort();}
...
void TetrysAcker::delete_known_and_affected_coded_and_lost(set<int> packets_to_remove){
...
TetrysACKRx::TetrysACKRx()
{
	arq_tx_=0;
} //end of constructor


int TetrysACKRx::command(int argc, const char*const* argv)
{
	Tcl& tcl = Tcl::instance();
  if (argc == 3) {
		if (strcmp(argv[1], "attach-TetrysTx") == 0) {
...
void TetrysACKRx::recv(Packet* p, Handler* h)
{
