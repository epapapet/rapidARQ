...
if {$arg_cnt != 14} {
    puts "# usage: ns <scriptfile> <bandwidth> <propagation_delay> <window_size> <cbr_rate> <pkt_size> <err_rate> <ack_rate> <num_rtx> <rate_k> <coding_depth> <timeout> <simulation_time> <seed> <experiment_id>"
...
set receiver [$ns link-arq $window $apppktSize $rate_k $cod_dpth $timeout_period $num_rtx $n1 $n3 [lindex $argv 12] [lindex $argv 6] [lindex $argv 13]]
...
$ns at 0.0 "$receiver print-parameters [lindex $argv 5] [lindex $argv 6] [lindex $argv 11] [lindex $argv 12]"
$ns at 0.0 "$cbr start"
$ns at [lindex $argv 11] "$cbr stop"
$ns at [expr {[lindex $argv 11] + 0.5}] "print_stats [lindex $argv 5] [lindex $argv 6] [lindex $argv 11] [lindex $argv 12]"
