...
void CaterpillarTx::recv(Packet* p, Handler* h)
{
	...
	if (pending) {
		fprintf(stderr, "Error at CaterpillarTx::recv, Tx should not have a pending frame when recv is called.\n");
		abort();
	}
...
void CaterpillarAcker::deliver_frames(int steps, bool mindgaps, Handler *h)
{
	...
	while (count < steps){
		...
		if ((status[((last_fwd_sn_+1)%sn_cnt)%wnd_] == RECEIVED) || (status[((last_fwd_sn_+1)%sn_cnt)%wnd_] == DECODED)){
			...
			if (pkt_buf[((last_fwd_sn_+1)%sn_cnt)%wnd_]){ fprintf(stderr, "Error at CaterpillarRx::deliver_frames, found a pkt with a status that is not RECEIVED nor DECODED.\n"); abort(); }
		}
...
void CaterpillarAcker::decode(Handler* h, bool afterCoded){
...
void CaterpillarAcker::clean_decoding_matrix(int from, int to)
{
	...
	if (deleted_lost_pkts) received_coded_cnt = 0; //if a lost frame is deleted (no more in CaterpillarTx's active window) then delete all coded pkts containing this lost frame
