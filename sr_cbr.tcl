...
if {$arg_cnt != 12} {
    puts "# usage: ns <scriptfile> <bandwidth> <propagation_delay> <window_size> <cbr_rate> <pkt_size> <err_rate> <ack_rate> <num_rtx> <timeout> <simulation_time> <seed> <experiment_id>"
...
set receiver [$ns link-arq $window $apppktSize $timeout_period $num_rtx $n1 $n3 [lindex $argv 10] [lindex $argv 6] [lindex $argv 11]]
...
$ns at 0.0 "$receiver print-parameters [lindex $argv 5] [lindex $argv 6] [lindex $argv 9] [lindex $argv 10]"
$ns at 0.0 "$cbr start"
$ns at [lindex $argv 9] "$cbr stop"
$ns at [expr {[lindex $argv 9] + 0.5}] "print_stats [lindex $argv 5] [lindex $argv 6] [lindex $argv 9] [lindex $argv 10]"
