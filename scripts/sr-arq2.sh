...
# Script for executing SR-ARQ experiment 2 multiple times
...
for rtx in "${num_rtx[@]}"; do
	for i in "${!delay[@]}"; do 
		for iter in $(seq 1 $NUM_ITERATIONS); do 
			...
			RUN_SIMULATION="${SIMULATOR_PATH} ${SIMULATION_FILE} 10M ${delay[$i]} ${wnd[$i]} 10M 1000 0.05 0.05 $rtx 0 1000 $iter"			
