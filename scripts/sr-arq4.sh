...
# Script for executing SR-ARQ experiment 4 multiple times
...
for rtx in "${num_rtx[@]}"; do
	for i in "${!bandwidth[@]}"; do 
		for iter in $(seq 1 $NUM_ITERATIONS); do 
			...
			RUN_SIMULATION="${SIMULATOR_PATH} ${SIMULATION_FILE} ${bandwidth[$i]} 100ms ${wnd[$i]} ${bandwidth[$i]} 1000 0.05 0.05 $rtx 0 1000 $iter"			
