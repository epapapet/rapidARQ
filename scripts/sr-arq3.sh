...
# Script for executing SR-ARQ experiment 3 multiple times
...
SIMULATOR_PATH="./ns"
SIMULATION_FILE="arq/sr_cbr.tcl"
EXPERIMENT_ID="3"
...
for rtx in "${num_rtx[@]}"; do
	for i in "${!err_rate[@]}"; do 
		for iter in $(seq 1 $NUM_ITERATIONS); do 
			echo "Experiment 3| num_rtx: $rtx Error rate: ${err_rate[$i]} Iteration $iter";
			RUN_SIMULATION="${SIMULATOR_PATH} ${SIMULATION_FILE} 10M 100ms 250 10M 1000 ${err_rate[$i]} ${err_rate[$i]} $rtx 0 1000 $iter"			
